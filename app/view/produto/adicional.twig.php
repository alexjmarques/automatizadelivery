...
<div class="osahan-checkout">
    ...
    <div class="p-3 osahan-cart-item osahan-home-page">
        <form method="post" id="form"
            action="{{BASE}}{{empresa.link_site}}/produto/addCarrinho/adicional/{{carrinho[':chave']}}" novalidate>
            ...
            <input type="hidden" name="valorFinal" id="valorFinal"
                value="{{ carrinho[':quantidade'] * carrinho[':valor }}">
            <input type="hidden" name="valor" id="valor" value="{{ carrinho[':valor }}">
            <div class="shadow bg-white rounded p-3 clearfix">
                ...
                <h6 class="font-weight-bold mb-0">Total <span class="float-right"> <span id="total">{{ moeda[':simbolo']
                            }} {{ (carrinho[':quantidade'] * carrinho[':valor'])|number_format(2, ',', '.')
                            }}</span></span></h6>
            </div>

            <button class="btn btn-success btn-block btn-lg addStyle">TURBINAR PEDIDO <i
                    class="feather-shopping-cart"></i></button>

            {% if produto[':tipoAdicional'] == 0 %}
            <p class="text-center mt-3 mb-3 addStyleModT" style="font-size: 18px;">ou</p>
            <a href="#FinalizarPedido" class="btn btn-success btn-block btn-lg addStyleMod" data-toggle="modal"
                data-target="#FinalizarPedido">Continuar sem turbinar</a>
            {% endif %}
